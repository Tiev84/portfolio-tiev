...
# --lam-lai : bỏ hẳn bản cài cũ rồi tải mới từ đầu.
# Thư mục cũ được ĐỔI TÊN để dành chứ không xoá — trong đó có thể còn việc
# bạn chưa kịp đăng lên web.
LAM_LAI=0
for __arg in "$@"; do
  case "$__arg" in
    --lam-lai|--reinstall) LAM_LAI=1 ;;
  esac
done
...
# ------------------------------------------------------------
# 1b. Dọn bản cài cũ (chỉ khi chạy với --lam-lai)
# ------------------------------------------------------------
if [ "$LAM_LAI" = "1" ]; then
  say "[dọn] Cài lại từ đầu"

  if [ -d "$DEFAULT_DIR/.git" ]; then
    # Còn việc chưa đăng lên web thì phải nói TRƯỚC khi đụng vào thư mục.
    CHUA_DAY="$(git -C "$DEFAULT_DIR" rev-list --count @{u}..HEAD 2>/dev/null || echo 0)"
    CHUA_LUU="$(git -C "$DEFAULT_DIR" status --porcelain 2>/dev/null | wc -l | tr -d " ")"

    if [ "$CHUA_DAY" != "0" ] || [ "$CHUA_LUU" != "0" ]; then
      warn "Bản cũ đang có việc CHƯA lên GitHub:"
      [ "$CHUA_DAY" != "0" ] && warn "   $CHUA_DAY lần lưu chưa đẩy lên"
      [ "$CHUA_LUU" != "0" ] && warn "   $CHUA_LUU file chưa lưu"
      warn "Thư mục sẽ được ĐỔI TÊN để dành, KHÔNG xoá — vẫn lấy lại được."
    fi

    CU="${DEFAULT_DIR}-cu-$(date +%Y%m%d-%H%M%S)"
    if mv "$DEFAULT_DIR" "$CU"; then
      ok "Đã cất bản cũ vào: $CU"
    else
      die "Không đổi tên được $DEFAULT_DIR.
Đóng app Portfolio Manager rồi chạy lại lệnh này."
    fi
  else
    ok "Chưa có bản cũ nào, cài mới hoàn toàn."
  fi

  # Gỡ app và shortcut cũ, để không còn cái nào trỏ vào thư mục vừa cất đi
  for __app in "/Applications/$APP_NAME.app" "$HOME/Applications/$APP_NAME.app"; do
    if [ -d "$__app" ]; then
      rm -rf "$__app" && ok "Đã gỡ $__app"
    fi
  done
  rm -f "$HOME/Desktop/$APP_NAME" "$HOME/Desktop/$APP_NAME.app" "$HOME/Desktop/$APP_NAME.command" 2>/dev/null
fi
...
if [ -f "$HERE/portfolio.html" ] && [ -d "$HERE/admin" ]; then
  ...
  if [ "$LAM_LAI" = "1" ]; then
    REPO="$DEFAULT_DIR"   # cài lại thì khỏi hỏi, dùng luôn chỗ cũ
  else
...
if [ "$LAM_LAI" = "1" ] && [ -n "${CU:-}" ]; then
  printf '  Bản cũ     : %s\n' "$CU"
  printf '               (giữ lại phòng khi cần, xoá được khi đã chắc chắn)\n'
fi
printf '\033[1m============================================================\033[0m\n\n'
